...
//verify successfull redirection to the lamding page after loggin in
    ...
    await expect.soft(page.locator("//button[@id='skipOnboarding']")).toBeEnabled();
    await page.click("//button[@id='skipOnboarding']");

    //verify that its impossible to proceed with onboarding without user specifying their goals with Nexura, i.e., "continue button is disabled"
    await expect.soft(page.locator("//button[@id='nextBtn']")).toBeDisabled();

    //iniate onboarding process for  "sell tickets"
    await page.locator("//div[@data-goal='sell-tickets']").click();
    await page.locator("//button[@id='nextBtn']").click();
    await page.fill("//input[@id='businessName']", "testbusiness");
